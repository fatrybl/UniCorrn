...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate UniCorrn on 7Scenes.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--dataset_dir", type=str, default="/projects/vig/Datasets/7Scenes/"
    )
    parser.add_argument(
        "--split",
        type=str,
...
def evaluate_unified(model, split="val", dataset_dir="/projects/vig/Datasets/7Scenes/"):
    ...
    val_dataset = SevenScenes2D3DHardPairDataset(
    ...
    for data_dict in tqdm(val_loader):
        ...
        if len(targets_gt.shape) == 2:
            targets_gt = targets_gt[None]
        ...
        # conf_queries = queries.squeeze().cpu().numpy()[conf_mask]
        ...
        if num_correspondences >= 4:
            ...
            # print(pcd_points.shape, transform.shape, estimated_transform.shape)
...
if __name__ == "__main__":
    ...
    directory = "results/2d3d/7scenes"
