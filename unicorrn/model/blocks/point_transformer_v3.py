...
try:
    ...
    print("Point Transformer. Flash attention is available.")
except ImportError:
    flash_attn = None
    ...
    def forward(self, input):
        for k, module in self._modules.items():
            ...
            if isinstance(module, PointModule):
                # print('debug0: PointModule')
                input = module(input)
            # Spconv module
            elif spconv.modules.is_spconv_module(module):
                if isinstance(input, Point):
                    # print('debug1: process sparse conv')
                    input.sparse_conv_feat = module(input.sparse_conv_feat)
                    input.feat = input.sparse_conv_feat.features
                    # print('debug2: input sparse_conv_feat assigned to input.feat')
                else:
                    # print('debug3: not a Point instance')
                    input = module(input)
            # PyTorch module
            else:
                if isinstance(input, Point):
                    input.feat = module(input.feat)
                    # print('debug4: process PyTorch module Point')
                    if "sparse_conv_feat" in input.keys():
                        input.sparse_conv_feat = input.sparse_conv_feat.replace_feature(
                            input.feat
                        )
                elif isinstance(input, spconv.SparseConvTensor):
                    # print('debug5: process PyTorch module SPCONV')
...
class SerializedAttention(PointModule):
    ...
    def get_rel_pos(self, point, order):
        ...
        if rel_pos_key not in point.keys():
            ...
            print('grid_coord', grid_coord.shape, order, K)
...
class SerializedAttentionRoPE(SerializedAttention):
    def __init__(
            self,
            channels,
            num_heads,
...
class Block(PointModule):
    def __init__(
        ...
        self.attn = SerializedAttentionRoPE(
            channels=channels,
...
class SerializedCatUnpooling(PointModule):
    def __init__(
            self,
            in_channels,
            skip_channels,
        ...
        super().__init__()
        self.cat_proj = PointSequential(nn.Linear(in_channels + skip_channels, out_channels))

        if norm_layer is not None:
            self.cat_proj.add(norm_layer(out_channels))

        if act_layer is not None:
            self.cat_proj.add(act_layer())
    ...
    def __init__(
        ...
        if not self.cls_mode:
            ...
            for s in reversed(range(num_dec_stages)):
                ...
                dec = PointSequential()
                skip_connection_index = s + (self.num_stages - num_dec_stages) - 1
                dec.add(
                    SerializedUnpooling(
                        in_channels=dec_channels[s + 1],
                        skip_channels=enc_channels[skip_connection_index],
