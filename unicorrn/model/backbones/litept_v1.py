...
class LitePT_Encoder(LitePT):
    """
    LitePT Encoder Wrapper class.

    Mirrors ``PTv3_Encoder``'s config surface so LitePT can be swapped in as the
    point-cloud backbone: ``feat_channels`` is the width of ``pcd["feat"]``
    (e.g. 4 for xyz+intensity) while the stem consumes ``in_channels``.

    ``enc_attn`` defaults to attention on every stage: LitePT only serializes when
    a stage uses attention (``forward`` gates on ``enc_attn[0]`` and ``GridPooling``
    re-serializes on ``enc_attn[s]``), and the downstream PTv3 upsampler blocks read
    the serialized order off the pooled parents.
    """
    ...
    def __init__(
        ...
        enc_attn = (True,) * num_stages if enc_attn is None else enc_attn
        enc_rope_freq = (100.0,) * num_stages if enc_rope_freq is None else enc_rope_freq

        super(LitePT_Encoder, self).__init__(
            in_channels=in_channels,
            order=order,
            stride=stride,
    ...
    def from_config(cls, cfg):
        return {
            ...
            "feat_channels": cfg.get("FEAT_CHANNELS", 4),
            "stride": cfg.STRIDE,
            "enc_depths": cfg.ENC_DEPTHS,
            "enc_channels": cfg.ENC_CHANNELS,
            "enc_num_head": cfg.ENC_NUM_HEADS,
            "enc_patch_size": cfg.ENC_PATCH_SIZE,
            "enc_conv": cfg.get("ENC_CONV", None),
            "enc_attn": cfg.get("ENC_ATTN", None),
            "enc_rope_freq": cfg.get("ENC_ROPE_FREQ", None),
    ...
    def forward(self, pcd, **kwargs):
        ...
        # UniCorrn hands the backbone an int32 grid; LitePT needs int64 because
        # GridPooling packs the batch index as ``grid_coord | batch << 48`` and
        # PointROPE indexes its rotary tables with the same coordinates.
        pcd["grid_coord"] = pcd["grid_coord"].long()

        output = super(LitePT_Encoder, self).forward(pcd)
