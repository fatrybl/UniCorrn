...
from .cropping import (extract_correspondences_from_pts3d, gen_random_crops, in2d_rect, crop_to_homography)
...
from .dust3r.datasets.utils.geometry import depthmap_to_absolute_camera_coordinates, geotrf, depthmap_to_camera_coordinates
...
class MASt3RBaseStereoViewDataset(BaseStereoViewDataset):
    ...
    def _trim_corrs(self, queries, targets):
        ...
        if length >= self.max_queries:
            ...
            return queries[mask], targets[mask]
        else:
            mask = np.random.choice(length, self.max_queries - length)
            return np.concatenate([queries, queries[mask]], axis=0), np.concatenate([targets, targets[mask]], axis=0)
    ...
    def __getitem__(self, idx):
        ...
        if self.n_corres == 'all' or (self.n_corres > 0 and ('corres' not in view)):
            if self.n_corres == 'all':
                corres1, corres2 = extract_correspondences_from_pts3d(*views, None, self._rng, nneg=self.nneg)
                n = corres1.shape[0]
                valid = np.ones(n, dtype=bool)
            else:
                corres1, corres2, valid = extract_correspondences_from_pts3d(*views, self.n_corres, self._rng, nneg=self.nneg)
        ...
        # modification for MaPo training
        ...
        queries, targets = self._trim_corrs(queries, targets)
        ...
        if self.bidirectional:
            batch['queries'] = torch.from_numpy(np.concatenate([queries, targets], axis=0))
            batch['targets'] = torch.from_numpy(np.concatenate([targets, queries], axis=0))
            batch['norm_queries'] = torch.from_numpy(np.concatenate([norm_queries, norm_targets], axis=0)).float()
            batch['norm_targets'] = torch.from_numpy(np.concatenate([norm_targets, norm_queries], axis=0)).float()
        else:
