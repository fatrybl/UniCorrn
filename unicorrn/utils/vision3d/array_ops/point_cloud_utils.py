...
def random_sphere_crop(pcd, keep_ratio):
    """
    Source: https://github.com/prs-eth/OverlapPredator/blob/main/datasets/transforms.py#L152
    ...
    points_centered = pcd[:, :3] - centroid

    dist_from_plane = np.dot(points_centered, rand_xyz)
    if keep_ratio == 0.5:
        mask = dist_from_plane > 0
    else:
        mask = dist_from_plane > np.percentile(dist_from_plane, (1.0 - keep_ratio) * 100)
