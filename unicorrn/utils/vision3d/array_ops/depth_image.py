...
def back_project(
    ...
    Args:
        depth_mat (array): depth image (H, W, 4) - depth + RGB.
    ...
    height, width = depth_mat.shape[:2]
    coords = np.arange(height * width)
    u = coords % width
    v = coords // width
    depth_map = depth_mat.reshape(height * width, -1)
    z = depth_map[:, 0] / scaling_factor
    if depth_limit is not None:
        z[z > depth_limit] = 0.0
    x = (u - center_x) * z / focal_x
    y = (v - center_y) * z / focal_y
    points = np.stack([x, y, z], axis=-1)
    points = np.concatenate([points, depth_map[:, 1:]], axis=-1)

    output_list = []

    masks = z > 0
    if return_matrix:
        point_mat = points.reshape(height, width, -1)
